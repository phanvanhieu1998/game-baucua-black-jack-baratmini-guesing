const soundData = require('@/store/poker-sound.json');
...
class PokerSound {
  ...
  bgSound(){
    let bgSound = this.audio('night_casino', {
      autoplay: true,
      loop: true,
    });
    bgSound.play();
  ...
  chipBet(){
	...
	diceshake(){
    this.audio('diceshake').play();
  }

  playFoldSound(){
    this.audio('fold').play();
  }
...
const sound = new PokerSound(soundData);
